...
/**
 * Export objects of NoiseModelling into KML format. Modified version of
...
//    private List<TriMarkers> isoCountours = new ArrayList<>();
	...
	public KMLDocument(OutputStream outputStream) throws XMLStreamException {
		...
		xmlOut = streamWriterFactory.createXMLStreamWriter(new BufferedOutputStream(outputStream), "UTF-8");
	}
	...
	public void setInputCRS(String crs) throws CRSException, CoordinateOperationException {
		// Create a new CRSFactory, a necessary element to create a CRS without defining
		// one by one all its components
		CRSFactory cRSFactory = new CRSFactory();

		// Add the appropriate registry to the CRSFactory's registry manager. Here the
		// EPSG registry is used.
		RegistryManager registryManager = cRSFactory.getRegistryManager();
		registryManager.addRegistry(new EPSGRegistry());

		// CTS will read the EPSG registry seeking the 4326 code, when it finds it,
		// it will create a CoordinateReferenceSystem using the parameters found in the
		// registry.
		...
		if (crsKML instanceof GeodeticCRS && crsSource instanceof GeodeticCRS) {
			transform = CoordinateOperationFactory
					.createCoordinateOperations((GeodeticCRS) crsSource, (GeodeticCRS) crsKML).iterator().next();
		}
	}
	...
	public KMLDocument writeTopographic(List<Triangle> triVertices, List<Coordinate> vertices)
		...
		// Write geometry
		xmlOut.writeCharacters(KMLWriter.writeGeometry(JTSUtils.convert(geometryFactory.createMultiPolygon(polygons)),
				Double.NaN, wgs84Precision, false, KMLWriter.ALTITUDE_MODE_ABSOLUTE));
	...
	public KMLDocument writeBuildings(FastObstructionTest manager) throws XMLStreamException {
		...
		// Write geometry
		xmlOut.writeCharacters(KMLWriter.writeGeometry(
				JTSUtils.convert(geometryFactory.createMultiPolygon(polygons.toArray(new Polygon[polygons.size()]))),
				Double.NaN, wgs84Precision, true, KMLWriter.ALTITUDE_MODE_RELATIVETOGROUND));
		xmlOut.writeEndElement();// Write Placemark
		xmlOut.writeEndElement();// Folder
		return this;
	}
	...
	public KMLDocument writeRays(Collection<PropagationPath> rays) throws XMLStreamException {
		...
		for (PropagationPath line : rays) {
			...
			// Write geometry
			xmlOut.writeCharacters(KMLWriter.writeGeometry(JTSUtils.convert(lineString), Double.NaN, wgs84Precision,
					false, KMLWriter.ALTITUDE_MODE_ABSOLUTE));
			xmlOut.writeEndElement();// Write Placemark
		}
		xmlOut.writeEndElement();// Folder
		return this;
