 ...
 * Status as of 3/2016:
 * 
 * Looks like Gridcell is the basic top-level environment object and that an LPJ process
 * runs one gridcell. There is nothing in the gridcell (except for latitude and longitude,
 * which is a point anyway) that requires this to be a contiguous, square area of space,
 * so I (FV) am using a gridcell to represent an Ecological Response Unit (ERU) which is
 * potentially scattered around and can fit non-areal or areal contexts of different
 * topology.
 * 
 * Creating one gridcell per cell in a raster model is incredibly wasteful, so the basic
 * approach uses a map classifier to reduce the context to a smaller number of ERUs and
 * models each one as a gridcell.
 * 
 * When this is vetted and tested, Gridcell should be renamed EcologicalResponseUnit.
 * 
 */
public class Gridcell {
    ...
    /**
     * landcoverfrac is read in from land cover input file or from instruction file in
     * getlandcover().
     */
    ...
    /**
     * The reason why Gridcell has its own seed, rather than using for instance a single
     * global seed is to make it easier to compare results when for instance changing the
     * order in which the simulation proceeds. It also gets serialized together with the
     * rest of the Gridcell state to make it possible to get exactly identical results
     * after a restart.
     *
     * \see randfrac()
     */
    ...
    /**
     * Just initialize one per ERU and continue defining in the initialization loop by
     * submitting data from more spatial locations.
     * 
     * @param config
     */
    ...
    /*
     * ------------------------------------------------------------------------- ------
     * these are called during initial setup at each extent found to be part of this ERU.
     * After the whole context has been scanned, finish() is called to setup the land
     * cover fractions, define the overall soil type and finalize initialization.
     * ------------------------------------------------------------------------- ------
     */
    int[]                 lucBuffer    = new int[LandcoverType.values().length];
    double                minLat       = Double.NaN, maxLat = Double.NaN, minLon = Double.NaN,
            maxLon = Double.NaN;
    ...
    public boolean addExtent(ISpace extent, int spaceOffset, IConcept landcovertype, int soilCode) {
        ...
        if (type != null) {
            ...
            /*
             * use a majority rule although we should ensure that the soil code is unique
             * in a ERU.
             */
    ...
    /**
     * Compute statistics after all data are in and return true if the ERU should be used.
     * 
     * @return
     */
    public boolean finish() {
        ...
        setLCFractions(lucBuffer[0] / totcl, lucBuffer[1] / totcl, lucBuffer[2]
                / totcl, lucBuffer[3]
                        / totcl, lucBuffer[4] / totcl, lucBuffer[5] / totcl);
    ...
    public void dailyAccounting(List<PFT> pftlist) {
        ...
        // Updates daily climate parameters including growing degree day sums
        // and
        // exponential temperature response term (gtemp, see below). Maintains
        // monthly
        // and longer term records of variation in climate variables.
        // PFT-specific
        // degree-day sums in excess of damaging temperatures are also
        // calculated here.
        ...
        if (_configuration.getSchedule().isFirstDayOfSimulation()
                || _configuration.getSchedule().isFirstDayOfYear()) {
            ...
            if (_configuration.getSchedule().isFirstDayOfSimulation()) {
                ...
                && _configuration.getSchedule().julianDay() == _configuration.COLDEST_DAY_NHEMISPHERE
                || climate.lat < 0.0
                        && _configuration.getSchedule()
                                .julianDay() == _configuration.COLDEST_DAY_SHEMISPHERE) {
    ...
    public void setLCFractions(double urban, double crop, double pasture, double forest, double natural, double peatland) {
    ...
    void create_stand(LandcoverType landcover, int numpatch) {
        ...
        Stand s = new Stand(this, _configuration, _configuration
                .getPFTs(), this.soiltype, landcover, numpatch);
    ...
    public void set_sdatecalc_temp(GridcellPFT gridcellpft) {
        ...
        if (pft.ifsdautumn && pft.forceautumnsowing != ForceAutumnSowing.SPRINGSOWING) { // TeWW,TeRa:
            ...
            if (Utils
                    .dayinperiod(gridcellpft.sdatecalc_temp, climate.testday_temp, gridcellpft.hlimitdate_default)
                    && pft.forceautumnsowing != ForceAutumnSowing.AUTUMNSOWING) {
            ...
            if (pft.name.startsWith("TeCo"))
                gridcellpft.sdatecalc_temp = (int) (60.0 / 85.0
                        * (gridcellpft.last_springdate20 - climate.adjustlat)
    ...
    void calc_seasonality() {
        ...
        if (var_prec <= 0.4 && var_temp <= 0.010) // no seasonality
             ...
             * Temperature seasonality, always above 10 degrees
        ...
        else if (climate.mprec_petmin20 <= 0.5 && climate.mprec_petmax20 > 0.5
        ...
        else if (climate.mprec_petmin20 > 0.5 && climate.mprec_petmin20 <= 1.0
                && climate.mprec_petmax20 > 1.0)
        ...
        for (PFT pft : _configuration.getPFTs()) {
            ...
            if (pft.phenology == Phenology.CROPGREEN) {
                ...
                if (gridcellpft.sdatecalc_temp != -1) {
                    ...
                    if (!gridcellpft.wintertype && Utils.dayinperiod(swindow_temp[0], Utils
                            .stepfromdate(climate.coldestday, -100), climate.coldestday)) {
                        ...
                        if (Utils.dayinperiod(swindow_temp[1], Utils
                                .stepfromdate(climate.coldestday, -100), climate.coldestday))
                            swindow_temp[1] = climate.coldestday;
                    }

                    if (gridcellpft.wintertype && Utils.dayinperiod(swindow_temp[1], climate.coldestday, Utils
                            .stepfromdate(climate.coldestday, 100))) {

                        swindow_temp[1] = climate.coldestday;
                        // swindow_temp[1] = gridcellpft.sdatecalc_temp; //gives
                        // better yields, but sdate
                        // transition not smooth
                        if (Utils.dayinperiod(swindow_temp[0], climate.coldestday, Utils
                                .stepfromdate(climate.coldestday, 100)))
                            swindow_temp[0] = climate.coldestday;
                    }
                }
