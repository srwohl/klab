...
public class Shape extends AbstractExtent implements IShape {
    ...
    public boolean containsCoordinates(double x, double y) {
        ...
        return preparedShape == null
                ? shapeGeometry.contains(Geospace.gFactory.createPoint(new Coordinate(x, y)))
    ...
    public Shape getSimplified(IQuantity resolution) {
        ...
        // convert to projection units. FIXME there's certainly a proper method that doesn't require reprojection (ask Andrea)
    ...
    protected IExtent contextualizeTo(IExtent other, IAnnotation constraint) {
        ...
        /*
         * dimensionality >= 2; if we have a grid, apply it. TODO this will need to be revised to
         * apply a 2D grid to 2D extents matched with a 3D grid, which we don't have for now.
         */
        Grid grid = null;
